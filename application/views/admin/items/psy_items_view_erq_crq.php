...
?>               
              ...
              <?php }
				  ...
				  else
				  {
					  ...
					  <table style="width:100%">
                      	<tr><td style="width:50%; font-size:18px"><?php echo  html_entity_decode($item->item_rubric_english);?></td><td class="urdufont-right" style="text-align:right"><?php echo $item->item_rubric_urdu;?></td></tr>
                      </table>
				  <?php 
				  }
			  }
    ...
    <div style="padding:10px 0px 10px 4px;">
    	<label for="item_comment_ae" class="col-sm-12 control-label">Assesment Expert <?php if(!empty($aeinfo)){echo '('.$aeinfo['firstname'].' '.$aeinfo['lastname'].')';}?> Comments <?php if($item->item_status_ae ==1){ echo '(Approved)'; } elseif($item->item_status_ae ==2){ echo '(Rejected)'; } ?> on <?php echo date("d-M-Y (h:i a)",strtotime($item->item_reviewed));?></label></label>
        <textarea id="item_comment_ae" name="item_comment_ae" rows="2" cols="55" style="width:100%" required="required" readonly="readonly"><?php echo $item->item_comment_ss; ?></textarea>
    </div>
      <?php if($item->item_status_psy ==0){?>
	  <?php echo form_open(base_url('admin/items/psy_submit_for_approval/'.$item->item_id), 'class="form-horizontal" enctype="multipart/form-data" ');  ?>  
    	<div style="padding-top:15px; margin-left:5px">
            <label for="item_comment_psy" class="col-sm-6 control-label">Comments (Psychometrician)</label>
            <textarea id="item_comment_psy" name="item_comment_psy" rows="2" cols="55" style="width:100%" required="required"></textarea>
            <input type="submit" name="submit_psy" id="submit_psy" value="Review" class="btn btn-success pull-right" style="margin:5px">
		</div>
     <?php echo form_close( );} else {?>
     	<div style="padding:10px 0px 10px 4px;">
                <label for="item_comment_psy" class="col-sm-6 control-label">Psychometrician <?php if(!empty($psyinfo)){echo '('.$psyinfo['firstname'].' '.$psyinfo['lastname'].')';}?> Comments <?php if($item->item_status_psy ==1){ echo '(Reviewed)'; } elseif($item->item_status_psy ==2){ echo '(Rejected)'; } ?> on <?php echo date("d-M-Y (h:i a)",strtotime($item->item_date_psy));?></label>
