  ...
  <div class="row">
  	<div class="col-8">
        ...
        <div class="row">
            ...
            <div class="col-lg-12 col-sm-12" style="font-weight:bold"> Item Writer Name : ___________<u><?php echo $item->firstname.' '.$item->lastname.'('.$item->username.')';?></u>__________</div>
...
?>
                    ...
                    <span style="padding-left:50px; font-weight:bold; font-size:20px"> Question :<?php echo html_entity_decode($item->item_stem_en);?></span>
...
?>               
    ...
    <div class="row col-lg-12">
        ...
        <textarea id="item_comment_ss" name="item_comment_ss" rows="2" cols="55" style="width:100%" required="required" readonly="readonly"><?php echo $item->item_comment_ss; ?></textarea>
            ...
            <div style="padding:10px 0px 10px 4px;">
                <label for="item_comment_ss" class="col-sm-6 control-label">Assessment Expert <?php if(!empty($aeinfo)){echo '('.$aeinfo['firstname'].' '.$aeinfo['lastname'].')';}?> Comments <?php if($item->item_status_ae ==1){ echo '(Approved)'; } elseif($item->item_status_ae ==2){ echo '(Rejected)'; } ?> on <?php echo $item->item_reviewed;?></label>
                <textarea id="item_comment_ss" name="item_comment_ss" rows="2" cols="55" style="width:100%" required="required" readonly="readonly"><?php echo date("d-M-Y (h:i a)",strtotime($item->item_reviewed)); ?></textarea>
            </div><?php }?>
            <?php if($item->item_comment_psy!=""){?>
            <div style="padding:10px 0px 10px 4px;">
                <label for="item_comment_ss" class="col-sm-6 control-label">Psychometrician <?php if(!empty($psyinfo)){echo '('.$psyinfo['firstname'].' '.$psyinfo['lastname'].')';}?> Comments <?php if($item->item_status_psy ==1){ echo '(Reviewed)'; } elseif($item->item_status_psy ==2){ echo '(Rejected)'; } ?> on <?php echo date("d-M-Y (h:i a)",strtotime($item->item_date_psy));?></label>
                <textarea id="item_comment_ss" name="item_comment_ss" rows="2" cols="55" style="width:100%" required="required" readonly="readonly"><?php echo $item->item_comment_psy; ?></textarea>
            </div>
            <?php }?>
             <?php if($item->item_status==1){?>
			<div style="padding:15px 0px; float:right"><a href="<?= base_url('admin/items/submit_for_approval/'.$item->item_id); ?>" class="btn btn-success"> Submit For Approval</a></div>
            <?php } ?>
