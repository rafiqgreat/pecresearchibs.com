...
?>
                    ...
                    <tr>
                        ...
                        <div><span style="margin-left:15px">سوال :</span> <?php echo html_entity_decode($item->item_stem_ur);?></div>
...
?>               
    ...
    <div class="row"  style="width:100%; margin-left:5px">
     ...
     <?php if($item->item_status_ss==0){?>
     <?php echo form_open(base_url('admin/items/ss_submit_for_approval/'.$item->item_id), 'class="form-horizontal" enctype="multipart/form-data" ');  ?>  
    	<div style="padding-top:15px; margin-left:5px">
            <label for="item_comment_ss" class="col-sm-6 control-label">Subject Specialist Comments</label>
            <textarea id="item_comment_ss" name="item_comment_ss" rows="4" cols="55" style="width:100%" required="required"><?php echo $item->item_comment_ss; ?></textarea>
            ...
            <a href=<?php echo base_url('admin/items/edit/'.$item->item_id); ?>><input type="button" name="edit" id="edit" value="Edit Item" class="btn btn-info pull-right" style="margin:5px"></a>
        </div>
     <?php echo form_close( );}  ?> 
      	<div style="padding-top:15px; margin-left:5px">
            <label for="" class="col-sm-12 control-label">Subject Specialist <?php if(!empty($ssinfo)){echo '('.$ssinfo['firstname'].' '.$ssinfo['lastname'].')';}?> Comments <?php if($item->item_status_ss ==1){ echo '(Rejected)'; } elseif($item->item_status_ss ==2){ echo '(Accepted with Change)'; }elseif($item->item_status_ss ==3){ echo '(Accepted without change)'; } ?> on <?php echo date("d-M-Y (h:i a)",strtotime($item->item_approved));?></label>
        <textarea id="" name="" rows="2" cols="55" style="width:100%" required="required" readonly="readonly"><?php echo $item->item_comment_ss; ?></textarea>
        </div>
        <?php if($item->item_comment_ae !=""){?>
        <div style="padding:10px 0px 10px 4px;">
                <label for="" class="col-sm-12 control-label">Assessment Expert <?php if(!empty($aeinfo)){echo '('.$aeinfo['firstname'].' '.$aeinfo['lastname'].')';}?> Comments <?php if($item->item_status_ae ==1){ echo '(Approved)'; } elseif($item->item_status_ae ==2){ echo '(Rejected)'; } ?> on <?php echo date("d-M-Y (h:i a)",strtotime($item->item_reviewed));?></label>
        ...
        <?php if($item->item_comment_psy !=""){?>
            <div style="padding:10px 0px 10px 4px;">
                <label for="" class="col-sm-6 control-label">Psychometrician <?php if(!empty($psyinfo)){echo '('.$psyinfo['firstname'].' '.$psyinfo['lastname'].')';}?> Comments <?php if($item->item_status_psy ==1){ echo '(Reviewed)'; } elseif($item->item_status_psy ==2){ echo '(Rejected)'; } ?> on <?php echo date("d-M-Y (h:i a)",strtotime($item->item_date_psy));?></label>
