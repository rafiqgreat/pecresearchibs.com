...
?>               
    ...
    <div class="row"  style="width:100%; margin-left:5px">
	 ...
	 echo form_open(base_url('admin/items/ae_submit_for_approval/'.$item->item_id), 'class="form-horizontal" enctype="multipart/form-data" ');  ?>  
    	<div style="padding-top:15px; margin-left:5px">
            ...
            <textarea id="item_comment_ae" name="item_comment_ae" rows="2" cols="55" style="width:100%" required="required"><?php echo $item->item_comment_ae; ?></textarea>
    ...
    <?php }?>
        ...
        <?php if($item->item_comment_psy !=""){?>
            <div style="padding:10px 0px 10px 4px;">
                <label for="item_comment_psy" class="col-sm-12 control-label">Psychometrician <?php if(!empty($aeinfo)){echo $psyinfo['firstname'].' '.$psyinfo['lastname'].')';}?> Comments <?php if($item->item_status_psy ==1){ echo '(Reviewed)'; } elseif($item->item_status_psy ==2){ echo '(Rejected)'; } ?> on <?php echo date("d-M-Y (h:i a)",strtotime($item->item_date_psy));?></label>
