...
class Itemsgroup extends MY_Controller
{
		...
		if($this->session->userdata('role_id')==3)
		{
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result){
					$data = array(
						'log_itemid' => $id,
						'log_title' => 'Itemsgroup submitted by Item Writer',
						'log_message' => 'Itemgroup {{log_itemid}} submitted by {{log_admin_id}} on {{log_date}}',
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result){
					$data = array(
						'log_itemid' => $id,
						'log_title' => 'Itemsgroup updated by Item Writer',
						'log_message' => 'Itemsgroup {{log_itemid}} updated by {{log_admin_id}} on {{log_date}}',
	...
	public function delete($id = 0)
	{
		...
		$this->session->set_flashdata('success', 'ItemsGroup has been deleted successfully!');
			$data = array(
				'log_itemid' => $id,
				'log_title' => 'Itemsgroup deleted',
				'log_message' => 'Itemsgroup {{log_itemid}} deleted by {{log_admin_id}} on {{log_date}}',
	...
	{ 
		...
		if($result){
			$data = array(
				'log_itemid' => $id,
				'log_title' => 'Itemsgroup submitted for approval',
				'log_message' => 'Itemsgroup {{log_itemid}} submitted by {{log_admin_id}} on {{log_date}}',
				'log_messagetype' =>'submitted',
				'log_admin_id' => $this->session->userdata('admin_id')
			);
			$log = $this->Itemsgroup_model->log_entry($data);
			$this->session->set_flashdata('success', 'ItemGroup has been deleted successfully!');
			redirect(base_url('admin/Itemsgroup'));
		}
		else{
	...
	public function ss_submit_for_approval($id=0)
	{
		...
		else
		{
			...
			if($result){
				$data = array(
					'log_itemid' => $id,
					'log_title' => 'Itemsgroup submitted',
					'log_message' => 'Itemsgroup {{log_itemid}} submitted by {{log_admin_id}} on {{log_date}}',
		...
		else
		{
			...
			if($result){
				$data = array(
					...
					'log_message' => 'Itemsgroup {{log_itemid}} approved by {{log_admin_id}} on {{log_date}}',
					...
					if($result)
						{
							...
							$data = array(
								'log_itemid' => $id,
								'log_title' => 'Itemsgroup reviewed by Item Reviewer',
								'log_message' => 'Item {{log_itemid}} reviewed by Item Reviewer {{log_admin_id}} on {{log_date}}',
								'log_messagetype' =>'rev_submitted',
								'log_admin_id' => $this->session->userdata('admin_id')
							);
							$log = $this->Itemsgroup_model->log_entry($data);
							$this->session->set_flashdata('success', 'Itemsgroup has been aceepted for piloting successfully!');
	...
	public function rev_ae_edit($id = 0)
	{
		...
		else
		{
			$data['title'] = 'Edit Group';
			$data['group'] = $this->Itemsgroup_model->get_group_rev_by_id($id);
			//echo '<pre>';	print_r($data['group']); echo '<hr />';
			$data['grades'] = $this->Itemsgroup_model->get_all_grades();
			if($this->session->userdata('role_id')!=4)
			{
				die('Alert! You are not authorized here');
