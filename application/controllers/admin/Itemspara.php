	...
	public function rev_ae_submit_for_approval($id=0)
	{
		if($this->session->userdata('role_id')==4)
		{
			if($this->input->post('submit'))
			{
				...
				$data['para_rev_ae_comment'] = $this->input->post('para_rev_ae_comment');
		...
		else
		{
			...
			if(empty($result_rev))
			{
				...
				if($result)
				{
					...
					/*log entry karain on exported items but not submitted*/
				}
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result){
					...
					if($this->session->userdata('role_id')==2)
					{redirect(base_url('admin/itemspara/ss_view/'.$id));}
					elseif($this->session->userdata('role_id')==3){redirect(base_url('admin/itemspara/view/'.$id));}
