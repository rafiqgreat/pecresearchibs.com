...
class Itemwriters extends MY_Controller {
			...
			if ( $this->form_validation->run() == FALSE ) {
				...
				if(!empty($_FILES['picture']['name']))
				{
					...
					if($result['status'] == 1){
						...
						$this->session->set_flashdata('error', $result['msg']);
						redirect(base_url('admin/itemwriters'), 'refresh');
					}
				}
				/////////////////////////////////////////////////////
				...
				//print_r($data);
				//die();
				$result = $this->Itemwriters_model->add_itemwriters( $data );
				if ( $result ) {
					$this->session->set_flashdata( 'success', 'Itemwriter added successfully!' );
	...
	function edit( $id = 0 ) {
		if ( $this->input->post( 'submit' ) ) 
		{
			...
			if ( $this->form_validation->run() == FALSE ) {
				$data[ 'itemwriters' ] = $this->Itemwriters_model->get_itemwriters_by_id( $id );
				$data[ 'view' ] = 'admin/itemwriters/itemwriters_edit';
				...
				$data_admin = array(
					...
					'lastname' => $this->input->post( 'lastname' ),
					'email' => $this->input->post( 'email' ),
					'address' => $this->input->post( 'placeofposting' ),
	...
	function delete( $id = 0 ) {
		...
		$this->session->set_flashdata( 'success', 'Itemwriter has been deleted successfully!' );
		redirect( base_url( 'admin/itemwriters' ) );
	}
	//---------------------------------------------------------------
	public
